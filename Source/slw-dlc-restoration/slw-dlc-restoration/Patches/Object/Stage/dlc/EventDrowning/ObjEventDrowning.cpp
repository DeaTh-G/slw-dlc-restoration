...
void slw_dlc_restoration::ObjEventDrowning::AddCallback(app::GameDocument& in_rDocument)
{
	app::ObjEventDrowning::AddCallback(in_rDocument);

	auto* pEffectCollider = GetComponent<app::game::GOCCollider>()->GetShapeById(1);
	pEffectCollider->SetLocalTranslation({ pEffectCollider->m_UnkMtx.GetTransVector() / 2.0f });
}
